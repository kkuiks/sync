...
public class TagService {
  ...
  public void addTagsToPost(Post post, @Nullable Project project, List<String> tags) {
    ...
    addNormalizedTagsToPost(post, project, filteredTags);
  }
  ...
  public void replaceTags(Post post, List<String> tags) {
    ...
    Set<String> currentTagNames =
        currentTags.stream().map(postTag -> postTag.getTag().getName()).collect(Collectors.toSet());

    for (PostTag postTag : currentTags) {
      if (!requestedTagNames.contains(postTag.getTag().getName())) {
        post.removeTag(postTag.getTag());
    ...
    addNormalizedTagsToPost(post, post.getProject(), tagsToAdd);
  }
  ...
  private void addNormalizedTagsToPost(
      Post post, @Nullable Project project, List<String> filteredTags) {
